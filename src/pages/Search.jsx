...
const Search = () => {
    ...
    useEffect(() => {
        const fetchItems = async () => {
            setLoading(true);
            try {
                const response = await axios.get(`https://reasype-backend.vercel.app/api/items`, {
                    params: {q: query}
                });
    ...
    <div className=' px-6 lg:px-12 py-20'>
        ...
        <div className='bg-white md:max-w-3xl mx-auto p-4 rounded relative flex items-center'>
            ...
            <input value={query} onChange={handleSearch} className='outline-none w-full placeholder:text-[#1b2629]' name='query' type="search" placeholder='' id='search' required=""/>
        </div>
